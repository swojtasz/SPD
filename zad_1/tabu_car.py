...
def get_cmax(self, order):
    if isinstance(order[0], int):
        jobs_in_order = deepcopy([self.jobs[order[i]] for i in order])
    else:
        jobs_in_order = deepcopy(order)
    prev_job = RPQ(0, 0, 0, 0)
    cmax = 0
    for job in jobs_in_order:
        if job.r <= prev_job.C:
            job.C = prev_job.C + job.p
        else:
            job.C = job.r + job.p
        if job.C + job.q > cmax:
            cmax = job.C + job.q
...
def tabu(self, init=None, generate=None, stop=None, neighbourhood_size=5, tabu_list_size=10):
    ...
    while iterations < max_iterations and time.time() - start_time < duration:
        ...
        cmax_of_candidates = [cmax for cmax in list(map(self.get_cmax, candidates))]
