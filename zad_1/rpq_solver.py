...
class RPQ_Instance:
    ...
    def load_from_file(file_path: str):
        ...
        instance.tasks = []
        instance.tasks_number, parameters_count = (int(val) for val in lines[0].split())
        i = 0
        for job in lines[1:1 + instance.tasks_number]:
            r, p, q = (int(val) for val in job.split())
            instance.tasks.append(RPQ_Task(i, r, p, q))
            i = i+1
...
def solver(file_name):
    ...
    test_instance = RPQ_Instance.load_from_file(file_name)
    t = Timer()
    t.start()
